...
if TYPE_CHECKING:
    from sci_cli.utils.clearml import RequirementsConfig
    ...
    **kwargs
    ...
    def before_instantiate_classes(self):
        ...
        if self.subcommand:
            ...
            try:
                import clearml.task
                from .utils.clearml import set_conf_env, fix_clearml_requirements
...
def run_cli(
    ...
    main file. It registers a custom OmegaConf resolver for the lightning logs directory
    `sci_cli.lightning_logs_dir` and configures the CLI with default settings.
    ...
    OmegaConf.register_new_resolver("sci_cli.lightning_logs_dir", lambda: str(main_filepath.parent))

    return CustomCLI(
        parser_kwargs={
            "fit": {
                "default_config_files": [
                    (main_filepath.parent / f"{main_filepath.stem}_fit_config.yml").as_posix()
                ],
            },
